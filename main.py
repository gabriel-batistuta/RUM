...
class GitHub_API_Handler:
    ...
    def _list_root_files(self, repo):
    ...
    def download_gitignore(self, gitignore:dict[str, Union[str, bytes]]):
        ext = 'Example.' + gitignore["extension"]
        if type(gitignore.get("content")) == str:
            with open(ext, 'w') as file:
                file.write(gitignore["content"])
        else:
            with open(ext, 'wb') as file:
                file.write(gitignore["content"])

    def get_gitignore(self, required_gitignore:str, repo:Repository):
        files = self._list_root_files(repo)
        gitignore_files = [file for file in files if file.strip().endswith('.gitignore') and file.strip() != '.gitignore']
        
        file_contents = []
        for i in gitignore_files:
            if required_gitignore == i:
                content = self.get_file_content(i, repo)
                self.download_gitignore(content)
                return
        for file in gitignore_files:
            content = self.get_file_content(file, repo)
...
if __name__ == '__main__':
    ...
    repo = api.find_repo('github/gitignore')
    api.get_gitignore('Python.gitignore',repo)